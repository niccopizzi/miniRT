...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/06/14 14:25:01 by npizzi            #+#    #+#             */
/*   Updated: 2025/06/14 14:39:03 by npizzi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	parse_rgb(char **line, t_color *color)
{
	...
	*color = _mm_load_ps(data);
	(*line) = (*line) + i;
	return (vector_in_range(*color, 0, 1));
}

bool	parse_vector_or_point(char **line, t_vec4 *value, float point,
		char *msg)
{
	float	data[4];
	...
	*line = ft_strtof(*line, &data[0]);
	if (**line != ',')
		return (printf("%s %s", msg, FORMAT_ERR), false);
	*line = *line + 1;
	if (!is_valid_float(*line))
		return (printf("%s %s", msg, INV_FLOAT), false);
