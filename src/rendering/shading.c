...
t_color     color_at_hit(t_shading* info, const t_light* l, const t_world* w)
{
    ...
    ret = w->ambient * info->obj_hit->color * info->obj_hit->material[AMBIENT];
                                ...
                                const t_light* l)
{
    ...
    shade_info->light_dir /= shade_info->distance;
    shade_info->front_face = (vector_dot_product(ray->direction,
                            shade_info->normal_at) < 0);
}
