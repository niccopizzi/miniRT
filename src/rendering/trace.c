...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/06/14 14:26:11 by npizzi            #+#    #+#             */
/*   Updated: 2025/06/14 14:26:34 by npizzi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		t_shading *info)
{
	...
	{
		t = START_VAL;
		if (obj_ptr[i].hit(&obj_ptr[i], &shadow_ray, &t) && t < info->distance)
			return (true);
		i++;
	}
	return (false);
...
t_color	get_light_contribution(const t_world *w, t_shading *info)
{
	...
	while (i < size)
	{
		...
		if (!is_in_shadow(info->point_hit + info->normal_at * (float)BIAS, w,
				info))
